...
Compile the plugin for each platform you wish to use.  Place the compiled plugins (*.dll, *.so) in Assets/Plugins.  In the inspector, set each plugin to be usable from that platform only (e.g. Set the Linux .so file to be runnable from Linux only).
...
To run a client, select "Build and Run" from the Build Settings window.  When the game runs, click "Be the client".
